   ...
   public function getDb() // methode permettant d'instancier la class PDO et de crée un objet PDO
   {
       if(!$this->db)//seulement si $thid->db n'est pas rempli, si il n'y a pas de connexion BDD, alors on la construit
       {
            try
            {
                   ...
                   if(isset($_GET['action']) && $_GET['action'] =="conducteur")
                  { $this->table = $xml->table; } // on associe le nom de la table du chier xml a la propriete $table de la class
               
                  elseif(isset($_GET['action']) && $_GET['action']=="vehicule")
                  { $this->table = $xml->table2; } // on associe le nom de la table du chier xml a la propriete $table de la class

               elseif(isset($_GET['action']) && $_GET['action']=="association_vehicule_conducteur")
                  { $this->table = $xml->table3; } // on associe le nom de la table du chier xml a la propriete $table de la class
               else{ $this->table = $xml->table; }
                    try
                    {
...
///-------------------------------------------------------------------------------------------------------------------///
   ...
   {  
    ...
    // $this->table: represente dasn notre cas la table conducteur, c'es ce que l'on a recupere du fichier config.xml
    //mais j'arrive pas faire le changement de table
...
///-------------------------------------------------------------------------------------------------------------------///
   ...
   public function getFields()//methode permettant de recupere le nom des champs/colonnes des  tables 
   {
...
///-------------------------------------------------------------------------------------------------------------------///
   ...
   public function select($id)
      {    //$q = $this->getDB()->query("SELECT*FROM conducteur WHERE id_conducteur = 1);
          $q = $this->getDB()->query("SELECT*FROM ".$this->table.' WHERE id_'.$this->table." = ".$id);
